...
describe('DIFFICULTY_CONFIG', () => {
  it('maps Easy to 8x8 grid with 8 mines', () => {
    expect(DIFFICULTY_CONFIG[Difficulty.Easy]).toEqual({ rows: 8, cols: 8, mines: 8 })
  })

  it('maps Medium to 12x12 grid with 16 mines', () => {
    expect(DIFFICULTY_CONFIG[Difficulty.Medium]).toEqual({ rows: 12, cols: 12, mines: 16 })
  })

  it('maps Hard to 16x16 grid with 32 mines', () => {
    expect(DIFFICULTY_CONFIG[Difficulty.Hard]).toEqual({ rows: 16, cols: 16, mines: 32 })
  })
  ...
  it('covers all Difficulty values', () => {
    ...
    expect(configuredDifficulties.sort()).toEqual(allDifficulties.sort())
  })
})
